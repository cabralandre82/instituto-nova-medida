...
Atualizado em: **2026-04-20** (pós-PR-053 / D-064 — persistência server-authoritative do consent LGPD no /api/checkout).
...
**Estado dos demais ALTOs:** fechados também 22.1 (ViaCEP), 22.2 (leads DoS), 13.1 (fetchWithTimeout — PR-042), **11.1 (Next bump + CVE-2025-29927 fix — PR-041)**, **13.2 (circuit breaker — PR-050)**, **5.5 (reconciliação pós-clawback — PR-051)**, **5.12 (asaas_events retention + PII redact — PR-052)**, **5.6 (checkout consent persistido — PR-053)**, LGPD 6.3/6.4/17.3/Art. 16. 14.1 foi rebaixado pra 🟡 PARCIAL pela D-057. Pendentes são não-AI: 5.8 (customer takeover no checkout/reserve sem auth), 17.4 (signed URL log).
